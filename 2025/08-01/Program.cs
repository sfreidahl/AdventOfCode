...
foreach (var pair in sortedPairs.Take(1000))
{
    ...
    if (pair.One.Circuit is not null)
    {
    ...
    if (pair.Two.Circuit is not null)
    {
...
var result = circuits.OrderByDescending(x => x.Count).Take(3).Aggregate(0, (acc, src) => acc == 0 ? src.Count : acc * src.Count);
